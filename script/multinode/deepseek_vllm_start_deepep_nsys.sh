...
CMD="VLLM_USE_V1=1 nsys profile --trace-fork-before-exec=true -t cuda,osrt,nvtx,cudnn,cublas --cuda-graph-trace=node --gpu-metrics-device=0 --delay 120 --duration 60 vllm serve /cfs/lionthu/models/deepseek-ai/DeepSeek-R1/ \
     --trust-remote-code \
     --max_model_len 12800 \
     --max-seq-len-to-capture 12800 \
