...
# cannot import envs directly because it depends on taco_llm,
#  which is not installed yet
envs = load_module_from_path('envs', os.path.join(ROOT_DIR, 'taco_llm', 'envs.py'))

VLLM_TARGET_DEVICE = envs.VLLM_TARGET_DEVICE

# taco_llm only supports Linux platform
assert sys.platform.startswith(
    "linux"), "taco_llm only supports Linux platform (including WSL)."
...
class CMakeExtension(Extension):
    ...
    def __init__(self, name: str, cmake_lists_dir: str = '.', **kwa) -> None:
        ...
        print(f"========cmake_lists_dir={cmake_lists_dir}")
...
class cmake_build_ext(build_ext):
    ...
    def configure(self, ext: CMakeExtension) -> None:
        ...
        if is_ninja_available():
            ...
            build_tool = []
        print(f"====cmkae cmd: {ext.cmake_lists_dir}, {build_tool}, {cmake_args}, {self.build_temp}")
    ...
    def build_extensions(self) -> None:
        ...
        for ext in self.extensions:
            ...
            print(f"========{ext.name}, {ext_target_name}")
...
def get_taco_llm_version() -> str:
    version = find_version(get_path("taco_llm", "__init__.py"))
...
if _is_cuda():
    ext_modules.append(CMakeExtension(name="taco_llm._moe_C"))

    if _install_punica():
        ext_modules.append(CMakeExtension(name="taco_llm._punica_C"))
...
package_data = {
    "taco_llm": [
        ...
        "dropout_layer_norm*.so", "py.typed"
    ]
}
...
setup(
    ...
    author="Tencent Cloud AI team",
    #  license="Apache 2.0",
    description=("A high-throughput and memory-efficient inference and "
                 "serving engine for LLMs"),
    long_description="""Taco stands for Tencent AI "C" optimizer.
    "C" has multiple explanations, among which "Cloud",
    "Comprehensive", "Compute", or "Compiler-assisted" are the popular ones
    that depict some aspects of TACO.
    Despite being an inference optimizer as this python package implements,
    TACO also provides training, serving, and cloud-native AI functionalities.
    Just go check them out!""",
    #  long_description_content_type="text/markdown",
    url="https://cloud.tencent.com/document/product/560/57679",
    # project_urls={
